...
function OrderTracking() {
  ...
  const lastOrder = React.useMemo(() => {
    try {
      return JSON.parse(localStorage.getItem('nexusLastOrder')) || {};
    } catch (error) {
      return {};
    }
  }, []);
  ...
  const fetchTracking = React.useCallback(
    async (payload, { silent = false } = {}) => {
      ...
      try {
        ...
        try {
          localStorage.setItem('nexusLastOrder', JSON.stringify({ orderNumber: data.orderNumber, email: data.email }));
        } catch (storageError) {
            ...
            <form onSubmit={handleSubmit}>
              <Stack spacing={2}>
                ...
                <Typography variant="caption" color="text.secondary">
                  Having trouble locating your ID? Search your inbox for the subject “Nexus Electronics Order Confirmation”.
                </Typography>
              </Stack>
            </form>
          </Grid>
          <Grid item xs={12} md={6}>
            <Stack spacing={2}>
              ...
              <Stack direction="row" spacing={1} alignItems="center">
                ...
                <Typography variant="caption" color="text.secondary">
                  support@nexuselectronics.io • +1 (833) 555-0195
                </Typography>
              </Stack>
