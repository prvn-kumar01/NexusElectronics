...
productSchema.pre('save', async function(next) {
    ...
    // Auto-generate SEO Description if not present or if base description changed
    if ((this.isNew || this.isModified('description')) && process.env.GROQ_API_KEY) {
        try {
            const seoDesc = await groqService.generateSeoDescription(this.name, this.description, this.category);
            if (seoDesc) {
                this.seoDescription = seoDesc;
            }
        } catch (err) {
            console.error('Error generating SEO description in pre-save hook:', err);
        }
    }
