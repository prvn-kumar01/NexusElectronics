 ...
 *   post:
 *     summary: Add a review and generate AI summary
 *     tags: [Products]
 *     parameters:
 *       - in: path
 *         name: id
...
router.post('/:id/reviews', async (req, res) => {
  try {
    ...
    const review = { rating, comment, userName: userName || 'Anonymous' };
    product.reviews.push(review);

    const newNumReviews = product.reviews.length;
    const newRatingSum = product.reviews.reduce((sum, r) => sum + r.rating, 0);
    ...
    // Trigger Groq summary asynchronously in background
    if (product.reviews.length >= 2) { // Summarize if at least 2 reviews
        groqService.summarizeReviews(product.reviews).then(async (summary) => {
            if (summary) {
                product.aiReviewSummary = summary;
                await product.save();
            }
 ...
 *   get:
 *     summary: Get AI recommended bundle for a product
 *     tags: [Products]
 *     parameters:
 *       - in: path
 *         name: id
...
router.get('/:id/bundle', async (req, res) => {
  try {
    ...
    const recommendedCategories = await groqService.recommendBundleCategories(product.name, product.category);
    if (!recommendedCategories || recommendedCategories.length === 0) {
        return res.json([]);
    }

    const regexCategories = recommendedCategories.map(c => new RegExp(c, 'i'));
    
    const bundleProducts = await Product.find({
        _id: { $ne: product._id },
        $or: [
            { category: { $in: regexCategories } },
            { name: { $in: regexCategories } }
        ]
    }).limit(3);

    const formatted = bundleProducts.map(p => ({
      ...p.toObject(),
      id: p._id,
    }));
